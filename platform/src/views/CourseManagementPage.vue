...
const fetchCourses = async () => {
  loading.value = true;
  try {
    // API: GET /api/admin/course/course-page
...
const reviewCourse = async (id, status) => {
  // API: PUT /api/admin/course/{id}/review
  try {
...
const toggleRecommend = async (course) => {
  // API: PUT /api/admin/course/{id}/recommend
  try {
    ...
    course.recommend = course.recommend === 1 ? 0 : 1; // 失败后回滚
  }
};

const deleteCourse = async (id) => {
  if (!confirm('确定要删除该课程吗？')) return;
  // API: DELETE /api/admin/course/{courseId}
  try {
...
const fetchCategories = async () => {
  // API: GET /api/admin/course/category
  try {
...
const createCategory = async () => {
  ...
  if (categoryName) {
    // API: POST /api/admin/course/category/create
    try {
...
const updateCategory = async (category) => {
  ...
  if (newName) {
    // API: PUT /api/admin/course/category/{id}
    try {
...
const deleteCategory = async (id) => {
  ...
  // API: DELETE /api/admin/course/category/{id}
  try {
